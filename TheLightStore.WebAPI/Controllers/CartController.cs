using TheLightStore.Application.DTOs.Cart;
...
public class CartController : ControllerBase
{
    ...
    public async Task<IActionResult> AddToCart([FromBody] AddToCartRequest request)
    {
        try
        {
            ...
            {
                _logger.LogWarning("🚫 CART: Invalid model state: {@ModelState}", ModelState);
                return BadRequest(new { success = false, message = "Invalid request", errors = ModelState });
            }

            var (userId, sessionId) = GetUserAndSessionInfo();
            _logger.LogInformation("🛒 CART REQUEST: Adding to cart - UserId: {UserId}, SessionId: {SessionId}, ProductId: {ProductId}, VariantId: {VariantId}, Quantity: {Quantity}", 
                userId, sessionId, request.ProductId, request.VariantId, request.Quantity);
            
            _logger.LogInformation("🔍 CART: About to call AddToCartAsync service method");
            var result = await _cartItemService.AddToCartAsync(userId, sessionId, request.ProductId, request.Quantity, request.VariantId);
            _logger.LogInformation("🔍 CART: AddToCartAsync returned. Success: {Success}, Message: {Message}", result.Success, result.Message);

            if (!result.Success)
            {
                _logger.LogWarning("🚫 CART: Service returned failure - Message: {Message}, Errors: {@Errors}", 
                    result.Message, result.Errors);
                return BadRequest(new { success = false, message = result.Message, errors = result.Errors });
            }

            _logger.LogInformation("✅ CART: Successfully added item to cart - UserId: {UserId}, ProductId: {ProductId}", 
                userId, request.ProductId);
            return Ok(new { success = true, data = result.Data, message = result.Message });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "❌ CART CONTROLLER ERROR: Exception in AddToCart - UserId: {UserId}, ProductId: {ProductId}, VariantId: {VariantId}", 
                GetUserId(), request?.ProductId, request?.VariantId);
            return StatusCode(500, new { success = false, message = "Internal server error" });
        }
    }
