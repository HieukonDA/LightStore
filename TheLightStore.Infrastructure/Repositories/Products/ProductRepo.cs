...
public class ProductRepo : IProductRepo
{
    ...
    // 🔥 Admin method - lấy tất cả products (kể cả inactive)
    public async Task<PagedResult<Product>> GetAllForAdminAsync(PagedRequest request)
    {
        var query = _context.Products // ✅ Không filter IsActive
       .Include(c => c.Category)
       .Include(c => c.Brand)
       .Include(c => c.ProductImages.Where(img => img.IsPrimary == true))
       .Include(c => c.ProductReviews) // ✅ Include tất cả reviews cho admin
       .Include(c => c.ProductVariants) // ✅ Include tất cả variants (kể cả inactive)
       ...
       .AsQueryable();
        ...
        if (!string.IsNullOrEmpty(request.Sort))
        {
            switch (request.Sort.ToLower())
            {
                ...
                case "createdat":
                    ...
                    break;
                case "isactive":
                    query = query.OrderByDescending(c => c.IsActive).ThenBy(c => c.Name);
                    break;
                default:
                    query = query.OrderBy(c => c.Name);
                    break;
        ...
        {
            query = query.OrderByDescending(c => c.IsActive).ThenBy(c => c.Name); // Active trước
        }
    ...
    public async Task<Product?> GetByIdAsync(int id, bool includeRelated = false)
    {
        var query = _context.Products
            ...
            .Include(c => c.ProductVariants.Where(v => v.IsActive)) // ✅ Include active variants
                .ThenInclude(v => v.ProductImages) // ✅ Include variant images
            .Include(c => c.ProductVariants.Where(v => v.IsActive)) // ✅ Include variants again for attributes
                .ThenInclude(v => v.ProductVariantAttributes)
                .ThenInclude(a => a.Attribute)
            .Include(c => c.ProductVariants.Where(v => v.IsActive))
                .ThenInclude(v => v.ProductVariantAttributes)
                .ThenInclude(a => a.Value)
            .Include(c => c.ProductAttributes) // ✅ Include product level attributes
                .ThenInclude(a => a.Attribute)
    ...
    // 🔥 Admin method - lấy product bất kể trạng thái (active/inactive)
    public async Task<Product?> GetByIdForAdminAsync(int id, bool includeRelated = false)
    {
        var query = _context.Products
            .Where(c => c.Id == id) // ✅ Không filter IsActive
            .Include(c => c.Category)
            .Include(c => c.Brand)
            .Include(c => c.ProductImages) // ✅ Include all images
            .Include(c => c.ProductReviews) // ✅ Include tất cả reviews (kể cả pending/rejected)
            .Include(c => c.ProductVariants) // ✅ Include tất cả variants (kể cả inactive)
                .ThenInclude(v => v.ProductImages)
            .Include(c => c.ProductVariants)
                .ThenInclude(v => v.ProductVariantAttributes)
    ...
    // 🔥 Admin method - kiểm tra slug exist bất kể trạng thái
    public async Task<bool> SlugExistsForAdminAsync(string slug, int? excludeId = null)
    {
        return await _context.Products.AnyAsync(c => c.Slug == slug && c.Id != excludeId);
    }
    ...
    public async Task<PagedResult<Product>> GetByCategoryAsync(int categoryId, PagedRequest pagedRequest)
    {
        ...
        if (!string.IsNullOrEmpty(pagedRequest.Search))
        {
            query = query.Where(c => c.Name.Contains(pagedRequest.Search) ||
                                    (c.Description != null && c.Description.Contains(pagedRequest.Search)));
        }
    ...
    public async Task<PagedResult<Product>> GetByCategorySlugAsync(string categorySlug, PagedRequest pagedRequest)
    {
        ...
        if (!string.IsNullOrEmpty(pagedRequest.Search))
        {
            query = query.Where(p => p.Name.Contains(pagedRequest.Search) ||
                                    (p.Description != null && p.Description.Contains(pagedRequest.Search)));
        }
    ...
    public async Task<PagedResult<Product>> GetFeaturedAsync(PagedRequest pagedRequest)
    {
        ...
        {
            query = query.Where(p => p.Name.Contains(pagedRequest.Search) ||
                                   (p.ShortDescription != null && p.ShortDescription.Contains(pagedRequest.Search)));
        }
    ...
    public async Task<Dictionary<string, int>> GetSearchFiltersAsync(string query)
    {
        try
        {
            ...
            var brandFilters = await baseQuery
                .Include(p => p.Brand)
                .Where(p => p.Brand != null && p.Brand.Name != null)
