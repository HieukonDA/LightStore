...
namespace TheLightStore.Infrastructure.Repositories.Cart;
...
public class CartItemRepo : ICartItemRepo
{
    ...
    public async Task<CartItem> CreateAsync(CartItem cartItem)
    {
        try
        {
            // Log cartItem details before saving
            Console.WriteLine($"🛒 REPO: Creating CartItem - CartId: {cartItem.CartId}, ProductId: {cartItem.ProductId}, VariantId: {cartItem.VariantId}, Quantity: {cartItem.Quantity}, UnitPrice: {cartItem.UnitPrice}");
            
            _context.CartItems.Add(cartItem);
            
            Console.WriteLine($"🛒 REPO: Added to context, calling SaveChangesAsync...");
            await _context.SaveChangesAsync();
            
            Console.WriteLine($"✅ REPO: Successfully saved CartItem with ID: {cartItem.Id}");
            return cartItem;
        }
        catch (System.Exception ex)
        {
            Console.WriteLine($"❌ REPO ERROR: Failed to create cart item - Exception: {ex.GetType().Name}");
            Console.WriteLine($"❌ REPO ERROR: Message: {ex.Message}");
            
            if (ex.InnerException != null)
            {
                Console.WriteLine($"❌ REPO INNER: {ex.InnerException.GetType().Name} - {ex.InnerException.Message}");
            }
            
            throw new Exception($"Error creating cart item: {ex.Message}", ex);
        }
    }
